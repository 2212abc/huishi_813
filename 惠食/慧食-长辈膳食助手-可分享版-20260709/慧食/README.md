...
- 注册和旧账号重新同意都会在服务器记录隐私政策/用户协议版本和时间。
- 未确认当前版本时，账号可登录、导出和删除已有数据，但不能新写入健康档案或餐食数据。
- 档案页可导出账号、健康档案、餐食记录、家人关系和审计事件；不导出密码哈希或会话令牌。
- 销户必须再次输入当前密码，会级联删除账号、会话、档案、餐食和家人绑定。

`privacy.html` 和 `terms.html` 是可执行草案，但正式发布前必须补充运营主体、隐私联系方式和管辖条款，并完成法律审核。生产启动会要求 `LEGAL_DOCUMENTS_APPROVED=true`、`LEGAL_OPERATOR_NAME` 和 `LEGAL_CONTACT`。

## 备份、监控和审计

- `node ops/backup.js`：创建 SQLite 一致性快照，执行完整性检查后用 AES-256-GCM 加密。
- `node ops/restore-check.js <backup>`：在临时目录解密并校验，不覆盖生产库。
- `/healthz`：进程存活；`/readyz`：数据库可读且完整。
- 审计表只保存事件、结果、有限元数据和不可逆哈希，默认保留 180 天。

安装、systemd 定时器、恢复演练和当前线上故障见 `ops/PRODUCTION_RUNBOOK.md`。P0 逐项状态与责任人见 `P0_READINESS.md`。
...
- `tests/`：安全和行为回归测试
- `ops/`：加密备份、恢复校验、监控和 systemd 单元
- `compliance/`：专业规则审核、第三方清单和真实长辈测试方案
- `P0_READINESS.md`：P0 状态、阻塞项、责任人和所需输入
- `.env.example`：配置模板
