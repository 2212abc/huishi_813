...
长辈在“家人绑定”中生成 10 分钟有效的一次性 6 位码，家人登录自己的账号、切换到家人身份后输入该码。绑定关系保存在 `family_relations` 表，双方都可以查看脱敏账号信息并解除绑定；绑定码只保存 HMAC 摘要，使用一次后立即失效。

健康档案保存在 `health_profiles` 表，餐食及红黄绿提醒保存在 `meal_records` 表。绑定家人可以在自己的设备读取长辈主动记录的数据并同步“已查看”状态；未绑定账号不能指定其他用户编号读取数据，解除绑定后访问权限立即失效。本机 `localStorage` 只作为当前账号的断网副本，登录其他账号时不会沿用上一账号的健康数据。
